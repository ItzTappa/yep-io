...
export class Entity {
    constructor(x, y, type) {
        ...
        this.points = 0; this.upgradeProgress = 0; 
        this.kills = 0; this.upgradeCount = 0; 
        ...
        this.health = this.maxHealth; this.fireCooldown = 0;
        this.name = "Entity"; this.isDead = false; this.inSafeZone = false; 
        ...
        this.activeAbility = null; this.abilityCooldown = 0; this.abilityTimer = 0; this.abilityMaxCooldown = 450; 
    }
    ...
    applyUpgrade(upgradeId) {
        ...
        if (upgradeId === 'shield' || upgradeId === 'overdrive') {
            ...
            this.upgrades['shield'] = 0; this.upgrades['overdrive'] = 0;
    ...
    update() {
        ...
        if (this.regen > 0 && Math.random() < 0.05) this.health = Math.min(this.maxHealth, this.health + this.regen); 
        ...
        for (let i = this.trail.length - 1; i >= 0; i--) {
            let t = this.trail[i];
            if (t.type === 'smoke') {
                t.x += t.vx; t.y += t.vy; t.angle += t.rotSpeed; t.vx *= 0.9; t.vy *= 0.9; t.alpha -= 0.03; 
            } else if (t.type === 'fire') {
                t.x += t.vx; t.y += t.vy; t.alpha -= 0.12; 
            } else { 
                t.alpha -= 0.05; 
            }
    ...
    draw(ctx) {
        ...
        if (this.abilityTimer > 0 && this.activeAbility === 'shield') {
            ...
            ctx.strokeStyle = 'rgba(0, 200, 255, 0.8)'; ctx.lineWidth = 3; ctx.stroke();
            ctx.restore();
        }
        ...
        if (this.abilityTimer > 0 && this.activeAbility === 'overdrive') {
            ...
            ctx.strokeStyle = 'rgba(255, 50, 0, 0.8)'; ctx.lineWidth = 4;
            ctx.setLineDash([10, 10]); ctx.lineDashOffset = Date.now() / 20; ctx.stroke();
            ctx.restore();
        }
        ...
        if (this.auraVisual === 'regen') {
            ...
            ctx.strokeStyle = 'rgba(0, 255, 100, 0.4)'; ctx.lineWidth = 3;
            ctx.setLineDash([15, 15]); ctx.lineDashOffset = -Date.now() / 30; ctx.stroke();
            ctx.restore();
        }
        ...
        if (this.bodyVisual === 'armor') {
            ...
            ctx.strokeStyle = '#9ca3af'; ctx.lineWidth = armorThickness; ctx.beginPath();
        ...
        if (this.rearVisual === 'thrusters') {
            ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(this.angle);
            let tTier = Math.max(this.upgrades['speed'] || 0, this.upgrades['dash'] || 0);
            let tSize = 6 + tTier * 3; let tLen = 8 + tTier * 3;
            let backOffset = this.type === 'circle' ? -this.size : -this.size / 2;
            if (this.bodyVisual === 'armor') backOffset -= armorOffset; 
            let vSpread = this.type === 'square' ? this.size * 0.35 : this.size * 0.45;
            ctx.fillStyle = '#444';
        ...
        if (this.color === '#111111' || this.color === '#000000') {
            ctx.shadowBlur = 15; ctx.shadowColor = '#ffffff'; ctx.strokeStyle = 'rgba(255, 255, 255, 0.4)'; ctx.lineWidth = 2;
        }
        ...
        if (this.type === 'circle') { ctx.arc(0, 0, this.size, 0, Math.PI * 2); ctx.fill(); if (this.color === '#111111') ctx.stroke(); } 
            ...
            ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); ctx.fill(); if (this.color === '#111111') ctx.stroke();
        }
        ctx.restore();
        ...
        if (this.equipped.Skin && ITEMS_DB && ITEMS_DB[this.equipped.Skin]) {
            ...
            if (['spectre', 'luminescent', 'celestial', 'voidwalker', 'inferno', 'neon', 'dark', 'glitch'].includes(skinType)) {
                if (window.gameSettings.highQuality) { ctx.shadowBlur = 20; }
            }

            if (skinType === 'ghost') {
                ctx.fillStyle = 'rgba(255, 255, 255, 0.6)'; ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI*2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size/2, -this.size*0.866); ctx.lineTo(-this.size/2, this.size*0.866); ctx.closePath(); }
                ctx.fill(); ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(this.size*0.4, -this.size*0.3, 3, 0, Math.PI*2); ctx.arc(this.size*0.4, this.size*0.3, 3, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'assassin') {
                ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size*0.8, 0, Math.PI*2); else if (this.type === 'square') ctx.rect(-this.size*0.4, -this.size*0.4, this.size*0.8, this.size*0.8); else { ctx.moveTo(this.size*0.8, 0); ctx.lineTo(-this.size*0.4, -this.size*0.69); ctx.lineTo(-this.size*0.4, this.size*0.69); ctx.closePath(); }
                ctx.fill(); ctx.fillStyle = '#ff0000'; ctx.shadowColor = '#ff0000'; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(this.size*0.3, 0, 4, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'paladin') {
                ctx.strokeStyle = '#fbbf24'; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(-this.size*0.8, 0); ctx.lineTo(this.size*0.8, 0); ctx.moveTo(0, -this.size*0.8); ctx.lineTo(0, this.size*0.8); ctx.stroke();
            }
            else if (skinType === 'hologram') {
                ctx.fillStyle = 'transparent'; ctx.strokeStyle = '#00ffff'; ctx.lineWidth = 2; ctx.shadowColor = '#00ffff'; ctx.shadowBlur = 10; ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size*0.6, 0, Math.PI*2); else if (this.type === 'square') ctx.rect(-this.size*0.3, -this.size*0.3, this.size*0.6, this.size*0.6); else { ctx.moveTo(this.size*0.6, 0); ctx.lineTo(-this.size*0.3, -this.size*0.5); ctx.lineTo(-this.size*0.3, this.size*0.5); ctx.closePath(); }
                ctx.stroke();
            }
            else if (skinType === 'spartan') {
                ctx.strokeStyle = '#dc2626'; ctx.lineWidth = 6; ctx.beginPath(); ctx.moveTo(-this.size*0.5, -this.size*0.8); ctx.lineTo(this.size*0.5, 0); ctx.lineTo(-this.size*0.5, this.size*0.8); ctx.stroke();
            }
            else if (skinType === 'luminescent') {
                const hue = (Date.now() / 15) % 360; ctx.fillStyle = `hsl(${hue}, 100%, 50%)`; ctx.shadowColor = `hsl(${hue}, 100%, 50%)`; ctx.shadowBlur = 25; ctx.beginPath(); ctx.arc(0, 0, this.size*0.5, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'ninja') {
                ctx.fillStyle = '#111'; ctx.beginPath(); ctx.fillRect(-this.size, -this.size*0.3, this.size*2, this.size*0.6); ctx.fillStyle = '#fff'; ctx.fillRect(-this.size*0.5, -this.size*0.1, this.size, this.size*0.2);
            }
            else if (skinType === 'celestial') {
                ctx.fillStyle = '#fff'; ctx.shadowColor = '#00ffff'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.moveTo(0, -this.size*0.6); ctx.lineTo(this.size*0.2, -this.size*0.2); ctx.lineTo(this.size*0.6, 0); ctx.lineTo(this.size*0.2, this.size*0.2); ctx.lineTo(0, this.size*0.6); ctx.lineTo(-this.size*0.2, this.size*0.2); ctx.lineTo(-this.size*0.6, 0); ctx.lineTo(-this.size*0.2, -this.size*0.2); ctx.closePath(); ctx.fill();
            }
            else if (skinType === 'cyborg') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = '#9ca3af'; ctx.fillRect(-this.size, -this.size, this.size*2, this.size); ctx.fillStyle = '#ff0000'; ctx.shadowColor = '#ff0000'; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(this.size*0.4, -this.size*0.4, 5, 0, Math.PI*2); ctx.fill(); ctx.restore();
            }
            else if (skinType === 'voidwalker') {
                ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(0, 0, this.size*0.5, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle = '#a855f7'; ctx.lineWidth = 3; ctx.shadowColor = '#a855f7'; ctx.shadowBlur = 15; ctx.stroke();
            }
            else if (skinType === 'glitch') {
                ctx.fillStyle = 'rgba(0, 255, 255, 0.7)'; ctx.beginPath(); ctx.arc(-3, -3, this.size*0.4, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = 'rgba(255, 0, 255, 0.7)'; ctx.beginPath(); ctx.arc(3, 3, this.size*0.4, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'inferno') {
                ctx.fillStyle = '#fbbf24'; ctx.beginPath(); ctx.arc(0, 0, this.size*0.6, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#dc2626'; ctx.shadowColor = '#dc2626'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(0, 0, this.size*0.4, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'warlord') {
                ctx.strokeStyle = '#444'; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(-this.size*0.5, -this.size*0.5); ctx.lineTo(this.size*0.5, this.size*0.5); ctx.moveTo(this.size*0.5, -this.size*0.5); ctx.lineTo(-this.size*0.5, this.size*0.5); ctx.stroke();
            }
            else if (skinType === 'spectre') {
                ctx.fillStyle = 'rgba(168, 85, 247, 0.8)'; ctx.shadowColor = '#a855f7'; ctx.shadowBlur = 20; ctx.beginPath(); ctx.arc(0, 0, this.size*0.7, 0, Math.PI*2); ctx.fill();
            }
            else if (skinType === 'phantom') {
                ctx.strokeStyle = 'rgba(59, 130, 246, 0.8)'; ctx.lineWidth = 3; ctx.shadowColor = '#3b82f6'; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(0, 0, this.size*0.8, 0, Math.PI*2); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, this.size*0.4, 0, Math.PI*2); ctx.stroke();
            }
            else if (skinType === 'target') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.strokeStyle = 'rgba(255,255,255,0.6)'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(0, 0, this.size*0.6, 0, Math.PI*2); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, this.size*0.2, 0, Math.PI*2); ctx.stroke(); ctx.restore();
            }
            else if (skinType === 'stripes') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = 'rgba(255,255,255,0.3)'; for(let i = -this.size; i < this.size; i += 10) { ctx.fillRect(i, -this.size, 5, this.size*2); } ctx.restore();
            }
            else if (skinType === 'checker') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = 'rgba(0,0,0,0.3)'; let sq = this.size * 0.25; for(let x = -this.size; x <= this.size; x += sq) { for(let y = -this.size; y <= this.size; y += sq) { if (Math.abs(Math.round(x/sq) + Math.round(y/sq)) % 2 === 0) ctx.fillRect(x, y, sq, sq); } } ctx.restore();
            }
            else if (skinType === 'zebra') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = '#111'; for(let i = -this.size; i < this.size; i += 15) { ctx.beginPath(); ctx.moveTo(i, -this.size); ctx.lineTo(i + 5, 0); ctx.lineTo(i - 2, this.size); ctx.lineTo(i + 4, this.size); ctx.lineTo(i + 12, 0); ctx.lineTo(i + 8, -this.size); ctx.fill(); } ctx.restore();
            }
            else if (skinType === 'camo') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = '#4b5320'; ctx.fillRect(-this.size, -this.size, this.size*2, this.size*2); ctx.fillStyle = '#556b2f'; ctx.beginPath(); ctx.arc(-this.size*0.4, -this.size*0.3, this.size*0.5, 0, Math.PI*2); ctx.arc(this.size*0.3, this.size*0.5, this.size*0.4, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#8b7355'; ctx.beginPath(); ctx.arc(this.size*0.5, -this.size*0.4, this.size*0.4, 0, Math.PI*2); ctx.arc(-this.size*0.2, this.size*0.4, this.size*0.5, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = '#2f4f4f'; ctx.beginPath(); ctx.arc(-this.size*0.6, 0.1, this.size*0.3, 0, Math.PI*2); ctx.arc(this.size*0.1, -this.size*0.1, this.size*0.3, 0, Math.PI*2); ctx.fill(); ctx.restore();
            }
                ...
                ctx.strokeStyle = '#ffe600'; ctx.lineWidth = 3; ctx.shadowColor = '#ffe600'; ctx.shadowBlur = 10; ctx.beginPath(); ctx.ellipse(-this.size*0.2, 0, this.size*0.2, this.size*0.6, 0, 0, Math.PI*2); ctx.stroke();
            }
            else if (skinType === 'pirate') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.strokeStyle = '#111'; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(-this.size, -this.size); ctx.lineTo(this.size, this.size); ctx.stroke(); ctx.fillStyle = '#111'; ctx.beginPath(); ctx.arc(this.size*0.2, this.size*0.2, this.size*0.3, 0, Math.PI*2); ctx.fill(); ctx.restore();
            }
            else if (skinType === 'bandit') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.fillStyle = '#111'; ctx.fillRect(0, -this.size, this.size*0.6, this.size*2); ctx.fillStyle = '#fff'; ctx.fillRect(this.size*0.2, -this.size*0.4, this.size*0.2, this.size*0.2); ctx.fillRect(this.size*0.2, this.size*0.2, this.size*0.2, this.size*0.2); ctx.restore();
            }
            else if (skinType === 'mecha') {
                ctx.save(); ctx.beginPath();
                if (this.type === 'circle') ctx.arc(0, 0, this.size, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-this.size/2, -this.size/2, this.size, this.size); else { ctx.moveTo(this.size, 0); ctx.lineTo(-this.size / 2, -this.size * 0.866); ctx.lineTo(-this.size / 2, this.size * 0.866); ctx.closePath(); }
                ctx.clip(); ctx.strokeStyle = '#00ffcc'; ctx.lineWidth = 2; ctx.shadowColor = '#00ffcc'; ctx.shadowBlur = 5; ctx.beginPath(); ctx.moveTo(-this.size*0.8, -this.size*0.2); ctx.lineTo(0, -this.size*0.2); ctx.lineTo(this.size*0.2, -this.size*0.6); ctx.moveTo(-this.size*0.8, this.size*0.2); ctx.lineTo(0, this.size*0.2); ctx.lineTo(this.size*0.2, this.size*0.6); ctx.moveTo(this.size*0.4, -this.size*0.8); ctx.lineTo(this.size*0.4, this.size*0.8); ctx.stroke(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(0, -this.size*0.2, 3, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(0, this.size*0.2, 3, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(this.size*0.4, 0, 3, 0, Math.PI*2); ctx.fill(); ctx.restore();
            }
            else {
                ctx.beginPath();
                const innerSize = this.size * 0.5;
                if (this.type === 'circle') ctx.arc(0, 0, innerSize, 0, Math.PI * 2); else if (this.type === 'square') ctx.rect(-innerSize/2, -innerSize/2, innerSize, innerSize); else { ctx.moveTo(innerSize, 0); ctx.lineTo(-innerSize / 2, -innerSize * 0.866); ctx.lineTo(-innerSize / 2, innerSize * 0.866); ctx.closePath(); }
                if (skinType === 'neon') { ctx.fillStyle = '#ffffff'; if (window.gameSettings.highQuality) { ctx.shadowBlur = 15; ctx.shadowColor = '#ffffff'; } ctx.fill(); } else if (skinType === 'dark') { ctx.fillStyle = '#111111'; ctx.fill(); ctx.lineWidth = 3; ctx.strokeStyle = '#a855f7'; ctx.stroke(); } else if (skinType === 'gladiator') { ctx.fillStyle = '#9ca3af'; ctx.fill(); ctx.lineWidth = 2; ctx.strokeStyle = '#ffffff'; ctx.stroke(); }
            }
            ctx.restore();
        }
        ...
        if (this.isPlayer && !hideArrow && this.name !== "") {
            ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(this.angle); ctx.fillStyle = 'rgba(255, 255, 255, 0.4)'; ctx.beginPath();
            let arrowOffset = (this.type === 'square' ? this.size / 2 : this.size) + armorOffset + 4;
            ctx.moveTo(arrowOffset, -4); ctx.lineTo(arrowOffset + 8, 0); ctx.lineTo(arrowOffset, 4); ctx.fill(); ctx.restore();
        }
        ...
        if (!isMenuDummy) {
            ...
            if (this.dashCooldown <= 0) {
                ...
                ctx.fillStyle = '#ffffff'; const pct = Math.max(0, 1 - (this.dashCooldown / this.dashMaxCooldown)); const visualWidth = Math.min(40 * pct, 38);
            ...
            if (this.activeAbility) {
                ...
                if (this.abilityCooldown <= 0) {
                    ctx.fillStyle = '#ff00ff'; ctx.fillRect(this.x - 20, this.y + this.size + 28 + armorOffset, 40, 3); 
                } else {
                    ctx.fillStyle = '#ffffff'; const aPct = Math.max(0, 1 - (this.abilityCooldown / this.abilityMaxCooldown));
...
export class Bot extends Entity {
    constructor(x, y, type, startingPoints = 0) {
        ...
        this.dashTendency = Math.random(); this.strafeDir = Math.random() > 0.5 ? 1 : -1; this.personality = Math.random(); 
    ...
    updateBot(allPlayers, isCinematicIntro = false) {
        ...
        allPlayers.forEach(p => { 
            ...
            if (p === this || p.isDead || (this.isTeammate && (p.isPlayer || p.isTeammate)) || p.inSafeZone) return; 
        ...
        this.points += passiveGain; this.upgradeProgress += passiveGain;
        ...
        if (this.isTeammate && !nearestEnemy) {
            ...
            if (this.activeAbility && this.abilityCooldown <= 0 && (this.health < this.maxHealth * 0.3 || (this.activeAbility === 'overdrive' && trueDist < 300))) {
                this.useAbility();
            }
            ...
            if (this.health < this.maxHealth * fleeThreshold) {
                ...
                if (trueDist > optimalDist + 50) { 
                    this.vx += (dx / trueDist) * (currentSpeed * 0.12); this.vy += (dy / trueDist) * (currentSpeed * 0.12); 
                } else if (trueDist < optimalDist - 50) { 
                    this.vx -= (dx / trueDist) * (currentSpeed * 0.12); this.vy -= (dy / trueDist) * (currentSpeed * 0.12); 
                } else {
                    this.vx += (-dy / trueDist) * (currentSpeed * 0.08) * this.strafeDir; this.vy += (dx / trueDist) * (currentSpeed * 0.08) * this.strafeDir;
                }
            }
        } else {
...
export class SafeZone {
    ...
    update(player) {
        ...
        if (this.state === 'idle') {
            if (isInside) { this.state = 'triggering'; this.triggerTimer = 180; }
        } else if (this.state === 'triggering') {
            if (isInside) {
                this.triggerTimer--;
                if (this.triggerTimer <= 0) { this.state = 'active'; this.maxLifeTimer = Math.floor((Math.random() * 15 + 5) * 60); this.lifeTimer = this.maxLifeTimer; }
            } else {
                this.state = 'idle'; this.triggerTimer = 180;
            }
    ...
    draw(ctx) {
        ...
        if (this.state === 'triggering') {
            ...
            ctx.fillText(`ACTIVATING IN ${Math.ceil(this.triggerTimer / 60)}`, 0, -this.radius - 20);
        } else if (this.state === 'active') {
            ctx.fillStyle = '#ffe600'; ctx.font = 'bold 24px sans-serif'; ctx.textAlign = 'center';
            if (window.gameSettings.highQuality) { ctx.shadowBlur = 5; ctx.shadowColor = 'black'; }
            ctx.fillText(`SAFE: ${Math.ceil(this.lifeTimer / 60)}s`, 0, -this.radius - 20);
        }
        ctx.restore();
    }
}
...
export class Projectile {
    constructor(x, y, angle, owner, isMissile = false) {
        ...
        this.pierce = owner.pierceAmmo || 0; this.hitTargets = []; this.sizeScale = 1 + (owner.heavyArt * 0.5);
        
        if (isMissile) {
            this.speed = (owner.bulletSpeed || 15) * 0.45; this.damage = 10 + (owner.baseDamage * 0.5); 
            this.turnSpeed = 0.05 + (owner.missiles * 0.02); this.color = '#ff0044'; this.life = 80; 
        } else {
            this.speed = owner.bulletSpeed || 15; this.damage = owner.baseDamage; this.color = owner.color; this.life = 60 + (owner.sniper * 20); 
        }
    }
    ...
    update(targets) { 
        if (this.isMissile && targets) {
            ...
            targets.forEach(t => { 
                ...
                let d = distance(this.x, this.y, t.x, t.y); if (d < minDist && d < 600) { minDist = d; nearest = t; } 
            });
            if (nearest) {
            ...
            targets.forEach(t => { 
                ...
                let d = distance(this.x, this.y, t.x, t.y); if (d < minDist) { minDist = d; nearest = t; } 
            });
            if (nearest) {
                let targetAngle = Math.atan2(nearest.y - this.y, nearest.x - this.x);
                let diff = targetAngle - this.angle; 
                while (diff < -Math.PI) diff += Math.PI * 2; while (diff > Math.PI) diff -= Math.PI * 2;
                if (Math.abs(diff) < 0.4) this.angle += diff * 0.05; 
            }
        }
