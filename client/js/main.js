...
function renderLobbySlots(hostName = null) {
        ...
        if (p) {
            ...
            container.innerHTML += `
                <div class="player-slot ${p.ready ? 'ready' : ''}">
                    ...
                    <div class="name" style="display: flex; flex-direction: row; justify-content: center; align-items: baseline; gap: 6px;">
                        ${p.name} <span style="font-size:0.7rem; color:gray;">${roleTag}</span>
                    </div>
    ...
    // Toggle Ready/Unready button UI
    const readyBtn = document.getElementById('ready-btn');
    if (readyBtn) {
        if (me && me.ready) {
            readyBtn.innerText = "UNREADY";
            readyBtn.style.backgroundColor = "#ff4444";
...
document.addEventListener('click', async (e) => {
    ...
    if (target.id === 'ready-btn') {
        ...
        if (me) {
            ...
            if (isHost && everyoneElseReady && window.lobbyPlayers.length > 1 && !me.ready) {
                // START THE GAME!
