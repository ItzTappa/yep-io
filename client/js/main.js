...
function renderStats() {
    ...
    // NEW: List of abilities to check against to render pink ability badges
        ...
        if (match.upgrades && UPGRADE_POOL) {
            for(let key in match.upgrades) {
                ...
                if (tier > 0) {
                    ...
                    // IF IT IS AN ACTIVE ABILITY -> SHOW PINK BADGE
...
document.addEventListener('keydown', (e) => {
    if ((e.key === '/' || e.key === '`') && document.activeElement !== devInput) {
        ...
        if (devConsole && !devConsole.classList.contains('hidden')) {
            ...
            // IN-MATCH ESCAPE PRESSED: Kill player instantly to trigger Spectator Mode
